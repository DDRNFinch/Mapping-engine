(() => {
  ...
  function mergePromptPatch(patch, mode, courseId, routeId, editorStore) {
    const base = patch && typeof patch === 'object' ? patch : { v:2 };
    try {
      const extra = window.NaxosTaskPrompts?.compactPatch?.(mode, courseId, routeId, editorStore);
      if (Array.isArray(extra?.p) && extra.p.length) base.p = extra.p;
      if (Array.isArray(extra?.pc) && extra.pc.length) base.pc = extra.pc;
    } catch (error) {
      console.warn('Could not add Naxos task prompts to QR patch', error);
    }
  ...
  async function ksbPayload() {
    ...
    const store = readStore('ksb', course.id, 'default');
    const data = {
      ...
      promptMode:'activity-v1'
    };
    const patch = mergePromptPatch(compactStore(store), 'ksb', course.id, 'default', store);
  ...
  async function nvqPayload() {
    ...
    const store = readStore('nvq', courseId, route.id);
    const data = {
    ...
    const patch = mergePromptPatch(compactStore(store), 'nvq', courseId, route.id, store);
  ...
  window.NaxosEvidenceContract = Object.freeze({ version:3, file:CONTRACT_FILE, captureToken, compactStore });
})();